...
def generate_frames(video_path, dataset_name):
    ...
    while True:
        ...
        frame_info = {
            "file_name": f"frame_{frame_number:04d}.jpg",
    ...
    frames_df.to_csv(f"{dataset_name}_frames.csv", index=False)

    return frames_df

frames_info = generate_frames(video_source, dataset_name)

def save_frames_to_train_folder(frames_df, video_path, dataset_name):
    train_folder = os.path.join("/content/drive/My Drive", "datasets", dataset_name, "train")
    ...
    for index, row in frames_df.iterrows():
        ...
        if not success:
            print(f"Error reading frame {frame_name} from video")
            continue

        frame_save_path = os.path.join(train_folder, frame_name)
        cv2.imwrite(frame_save_path, frame)

    cap.release()

def adjust_images(frames_df, dataset_name):
    adjusted_folder = "adjusted"
    dataset_folder = os.path.join("/content/drive/My Drive", "datasets", dataset_name)
    adjusted_folder_path = os.path.join(dataset_folder, adjusted_folder)
    ...
    for index, row in frames_df.iterrows():
        ...
        if adjusted_frame is None:
            print("Error adjusting frame:", frame_name)
            continue

        adjusted_frame_path = os.path.join(adjusted_folder_path, frame_name)
        cv2.imwrite(adjusted_frame_path, adjusted_frame)

def mask_images(frames_df, dataset_name):
    mask_folder = "masked_images"
    dataset_folder = os.path.join("/content/drive/My Drive", "datasets", dataset_name)
    mask_folder_path = os.path.join(dataset_folder, mask_folder)
    ...
    for index, row in frames_df.iterrows():
        ...
        # Create a mask (example: circular mask)
        ...
        masked_frame_path = os.path.join(mask_folder_path, frame_name)
        cv2.imwrite(masked_frame_path, masked_frame)

def segment_images(frames_df, dataset_name):
    segmented_folder = "segmented_images"
    dataset_folder = os.path.join("/content/drive/My Drive", "datasets", dataset_name)
    segmented_folder_path = os.path.join(dataset_folder, segmented_folder)

    if not os.path.exists(segmented_folder_path):
        os.makedirs(segmented_folder_path)
    ...
    for index, row in frames_df.iterrows():
        ...
        # Apply smoothing (e.g., Gaussian blur) to the grayscale image
        blurred_frame = cv2.GaussianBlur(gray_frame, (5, 5), 0)

        # Apply segmentation algorithm (example: thresholding) to the smoothed image
        _, segmented_frame = cv2.threshold(blurred_frame, 25, 200, cv2.THRESH_BINARY)

        segmented_frame_path = os.path.join(segmented_folder_path, frame_name)
        cv2.imwrite(segmented_frame_path, segmented_frame)


save_frames_to_train_folder(frames_info, video_source, dataset_name)
adjust_images(frames_info, dataset_name)
mask_images(frames_info, dataset_name)
segment_images(frames_info, dataset_name)  # Updated function call

#Generate Bounding Boxes
def generate_bounding_boxes(frames_df, dataset_name):
    bounding_box_folder = "bounding_boxes"
    dataset_folder = os.path.join("/content/drive/My Drive", "datasets", dataset_name)
    bounding_box_folder_path = os.path.join(dataset_folder, bounding_box_folder)

    if not os.path.exists(bounding_box_folder_path):
        os.makedirs(bounding_box_folder_path)
    ...
    for index, row in frames_df.iterrows():
        ...
        edges = cv2.Canny(gray_frame, threshold1=30, threshold2=200)

        # Find contours in the edges
        contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        # Create a copy of the frame to draw bounding boxes
        frame_with_bounding_boxes = frame.copy()
        ...
        for contour in contours:
            ...
            cv2.rectangle(frame_with_bounding_boxes, (x, y), (x + w, y + h), (0, 255, 0), 2)

        bounding_box_frame_path = os.path.join(bounding_box_folder_path, frame_name)
        cv2.imwrite(bounding_box_frame_path, frame_with_bounding_boxes)
#---------------------------------------------------------------------------
def generate_contours(frames_df, dataset_name, epsilon=5):
    ...
    contours_list = []  # Initialize list to store contours for each frame
    ...
    for index, row in frames_df.iterrows():
        ...
        contours_list.append(valid_contours)

        # Create a copy of the frame to draw contours
        frame_with_contours = frame.copy()
        for contour in valid_contours:
            cv2.drawContours(frame_with_contours, [contour], 0, (255, 255, 100), 2)
